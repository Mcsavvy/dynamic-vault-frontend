        ...
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
                ...
                <div className="lg:col-span-2">
                    <div className="bg-card dark:bg-card border border-border rounded-lg p-6 mb-6">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                            ...
                            <div className="bg-muted p-3 rounded-md">
                                ...
                                <p className="text-xl font-bold text-foreground">${ASSET.initialPrice}</p>
                                <p className="text-xs text-muted-foreground">Mint price</p>
                            </div>
                            <div className="bg-muted p-3 rounded-md">
                                <p className="text-sm text-muted-foreground mb-1">Available</p>
                                <p className="text-xl font-bold text-foreground">{ASSET.availableSupply}</p>
                                <p className="text-xs text-muted-foreground">of {ASSET.totalSupply} tokens</p>
                            </div>
                            <div className="bg-muted p-3 rounded-md">
                                <p className="text-sm text-muted-foreground mb-1">Value/Token</p>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 mb-6">
                            {ASSET.isListed ? (
                                <>
                                    <Button 
                                        className="bg-teal-accent hover:bg-teal-accent/90 flex-1"
                                        onClick={() => {
                                            toast({
            ...
            <Tabs defaultValue="details" className="mb-12">
                ...
                <TabsContent value="details" className="bg-card dark:bg-card border border-border rounded-lg p-6">
                    <h2 className="text-2xl font-bold text-foreground mb-4">About this Asset</h2>
                    <p className="text-muted-foreground whitespace-pre-line mb-6">{ASSET.longDescription}</p>
                </TabsContent>
                <TabsContent value="price-history" className="bg-card dark:bg-card border border-border rounded-lg p-6">
                    <h2 className="text-2xl font-bold text-foreground mb-4">Price History</h2>
                    <div className="mb-6">
                        <div className="h-80 flex items-center justify-center bg-muted rounded-lg">
                            <p className="text-muted-foreground">Price history chart would be displayed here</p>
                        </div>
                    </div>
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-muted/50 border-b border-border">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-left font-medium text-muted-foreground">Previous</th>
                                    <th className="px-4 py-3 text-left font-medium text-muted-foreground">New Price</th>
                                ...
                                {PRICE_HISTORY.map((update, index) => {
                                    ...
                                    return (
                                        <tr key={index} className="hover:bg-muted/20">
                                            <td className="px-4 py-3">{update.timestamp}</td>
                                            <td className="px-4 py-3">${update.oldPrice}</td>
                                            <td className="px-4 py-3 font-medium">${update.newPrice}</td>
                                            <td className={`px-4 py-3 ${isPositive ? 'text-success-green' : 'text-alert-red'}`}>
                                                {isPositive ? '+' : ''}{percentChange}%
                                            </td>
                                            <td className="px-4 py-3 text-muted-foreground">{update.dataSource}</td>
                ...
                <TabsContent value="attributes" className="bg-card dark:bg-card border border-border rounded-lg p-6">
                    <h2 className="text-2xl font-bold text-foreground mb-4">Asset Attributes</h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        {ASSET.attributes.map((attr, index) => (
                            <div key={index} className="bg-muted p-4 rounded-lg">
                                <p className="text-sm text-muted-foreground mb-1">{attr.trait_type}</p>
                                <p className="font-semibold text-foreground">{attr.value}</p>
                            </div>
                        ))}
                    </div>
                </TabsContent>
                <TabsContent value="transactions" className="bg-card dark:bg-card border border-border rounded-lg p-6">
                            ...
                            <div key={index} className="flex items-center p-4 border border-border rounded-lg">
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <p className="font-bold text-foreground">${tx.price}</p>
                                    </div>
            ...
            <div className="bg-card dark:bg-card border border-border rounded-lg p-6">
                <h2 className="text-2xl font-bold text-foreground mb-6">Related Assets</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {[1, 2, 3].map((i) => (
                        <div key={i} className="bg-muted rounded-lg overflow-hidden">
                            ...
                            <div className="p-4">
                                <h3 className="font-semibold text-foreground">Related Asset {i}</h3>
                                <p className="text-sm text-muted-foreground mb-2">Short description of the related asset</p>
                                <p className="font-bold text-foreground">$85,000</p>
                            </div>
                        </div>
                    ))}
                </div>
